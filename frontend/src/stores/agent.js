  ...
  actions: {
    ...
    async loadSessions() {
      try {
        ...
        this.sessions = Array.isArray(response?.sessions)
          ? response.sessions
          : [];
        this.sortSessions();
      } catch (error) {
    ...
    async loadSessionMessages(sessionId) {
      try {
            ...
            type: isVideoMessage
              ...
              : attachments.length === 1
                ...
                : detectionResult
                  ...
                  : undefined,
            imageUrl:
              !isVideoMessage && attachments.length === 1
                ? attachments[0]
                : undefined,
            images:
              !isVideoMessage && attachments.length > 1
                ...
                : undefined,
            videoUrl:
              isVideoMessage && attachments.length ? attachments[0] : undefined,
            detectionResult,
        ...
        restoredMessages.forEach((message, index) => {
          ...
          const searchEnd =
            nextUserIndex === -1 ? restoredMessages.length : nextUserIndex;
          const resultMessage = restoredMessages
            .slice(index + 1, searchEnd)
            .find((item) => item.detectionResult?.source_video_url);

          if (resultMessage)
            message.videoUrl = resultMessage.detectionResult.source_video_url;
        });
    ...
    async deleteSession(sessionId) {
      try {
        ...
        this.sessions = this.sessions.filter(
          (session) =>
            String(this.getSessionKey(session)) !== String(sessionId),
        );
    ...
    async renameSession(sessionId, newTitle) {
      ...
      try {
        const response = await request.patch(`/chat/sessions/${sessionId}`, {
          title,
        });
    ...
    async togglePinSession(sessionId) {
      try {
        const response = await request.patch(`/chat/sessions/${sessionId}/pin`);
        const session = this.sessions.find(
          (item) => String(this.getSessionKey(item)) === String(sessionId),
        );

        if (session) {
          session.is_pinned = response?.is_pinned || false;
          this.sortSessions();
        }
