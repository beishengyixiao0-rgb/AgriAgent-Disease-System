...
import pytest
from app.agent.detection_agent import ALL_TOOLS
...
def test_multi_agent_routes_analysis_keywords_without_supervisor(monkeypatch):
    ...
    assert asyncio.run(collect_for("我检测了多少次")) == [
        {"type": "text_chunk", "content": "分析完成"}
    ]
    assert asyncio.run(collect_for("给出用户列表")) == [
        {"type": "text_chunk", "content": "分析完成"}
    ]
